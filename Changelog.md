...
- PR #800
   ...
   - Move passport.js into passport/message-list.js
...
- Modify webpack configurations (prod and dev) to work on server
  ...
  - Change entry point for server to be server/message-list.js
...
Previously in order to fetch data asynchronously on route change, we would specify a `static need` method in a `container` component, e.g. ``. This `need` method specified an action creator such as the method below.
...
- Move main route `all` to `server/message-list.js` from `server/config/routes.js`
